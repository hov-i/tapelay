...
http
  .createServer(async (req, res) => {
    ...
    // Error markers on the preview timeline: event.dateCreated spread across
    // the replay's own duration, offset from the replay's started_at above.
    // Mirrors GET /organizations/{org}/eventids/{event_id}/, which is what
    // sentry.mjs actually calls for a replay's (event-id-shaped) error_ids.
    const eventIdMatch = p.match(/^\/api\/0\/organizations\/acme\/eventids\/(e\d+)\/$/)
    if (eventIdMatch) {
      const n = Number(eventIdMatch[1].slice(1))
      const replayStartMs = Date.now() - 3600e3
      const offsetSec = (n * 137) % Math.max(1, durationSec - 1)
      return json(res, 200, { event: { dateCreated: new Date(replayStartMs + offsetSec * 1000).toISOString() } })
    }
