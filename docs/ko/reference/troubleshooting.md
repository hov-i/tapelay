...
| 변환이 느림 | 실시간 재생이라 그렇습니다. `구간 길이 ÷ 배속`만큼 걸리는 게 정상이지 버그가 아닙니다. 구간을 자르거나 `--speed`를 올려서 줄이세요. |
| 구간을 어떻게 잡아도, 세션 전체를 골라도 `No replayable FullSnapshot exists for that range`가 남 | 일부 Sentry SDK 버전이 밀리초 대신 초 단위 Unix 타임스탬프를 이벤트에 섞어 넣는 경우가 있습니다. 이러면 세션 길이가 실제보다 약 1000배(몇 분이 몇십 년으로) 부풀려져서, 요청한 구간이 실제 스냅샷 위치와 전혀 안 맞게 됩니다. tapelay 0.3.4부터 `fetchReplayEvents`가 이를 자동으로 감지해서 보정합니다 — 그보다 낮은 버전이면 업데이트하세요. |
