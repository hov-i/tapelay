...
토큰은 어느 경우든 만지지 않습니다(위 내용 참고). 녹화본 자체는 구간 미리보기 용도로만 페이지에 닿습니다 — 리플레이를 선택하면 서버가 그 rrweb 이벤트를 가져와 브라우저 탭으로 내려주고, Sentry 자체 리플레이 화면처럼 화면을 재생하면서 막대를 드래그해 구간을 지정할 수 있습니다. mm:ss 숫자를 추측해서 입력할 필요가 없어지는 대신, 이 과정은 전부 로컬호스트 안에서만 이뤄집니다. 실제 변환은 여전히 서버에서 처리하며, 브라우저는 디스크에 저장되는 파일이 아니라 미리보기용 사본만 봅니다.
